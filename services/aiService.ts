...
// nvidia/nemotron-3-ultra-550b-a55b:free — used for general/text generation
// (quiz gen for non-math subjects, flashcards, human-like drafts, general tutoring)
const MODEL_DEFAULT = "nvidia/nemotron-3-ultra-550b-a55b:free";
const MODEL_MATH = "openai/gpt-oss-20b:free";
const MODEL_CS = "openai/gpt-oss-20b:free";
...
const callAI = async (messages: any[], mockFallback: () => string, model: string = MODEL_DEFAULT) => {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      // Some reasoning models (e.g. Nemotron) can return content split across
      // `content` and a separate `reasoning` field, or occasionally emit an
      // empty `content` with the actual answer in `reasoning`. Prefer
      // `content`, but fall back to `reasoning` rather than throwing.
...
export const generateQuizQuestions = async (params: QuizParams): Promise<QuizQuestion[]> => {
  ...
  // Use Math model for math quizzes for better logic, default (Nemotron 3 Ultra) for others
